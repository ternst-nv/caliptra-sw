...
impl CertifyKeyExtendedMldsa87Cmd {
    ...
    pub(crate) fn execute(drivers: &mut Drivers) -> CaliptraResult<()> {
        ...
        // The mldsa variant of the certify key extended can be quite long, ~70,000,000 cycles
        // due to the multiple key generation and signing operations.  As such extend the watchdog
        // timer so the command can complete execution.
        caliptra_common::wdt::start_wdt(
            &mut drivers.soc_ifc,
            caliptra_common::WdtTimeout::new_const(200_000_000),
        );
